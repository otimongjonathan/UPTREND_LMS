                            ...
                            <tbody>
                                @forelse($collaterals as $collateral)
                                    <tr class="border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">{{ substr($collateral->description, 0, 50) }}...</td>
                                        ...
                                        <td class="px-6 py-3 flex gap-2">
                                            <a href="{{ route('collaterals.edit', [$loan, $collateral]) }}" class="text-blue-600 hover:underline text-xs">Edit</a>
                                            @if($collateral->status === 'pending')
                                                <form action="{{ route('collaterals.verify', [$loan, $collateral]) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    <button class="text-green-600 hover:underline text-xs">Verify</button>
                                                </form>
                                                <form action="{{ route('collaterals.reject', [$loan, $collateral]) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    <button class="text-red-600 hover:underline text-xs">Reject</button>
                                                </form>
                                            @endif
                                            <form action="{{ route('collaterals.destroy', [$loan, $collateral]) }}" method="POST" style="display:inline;" onsubmit="return confirm('Delete this collateral?');">
                                                @csrf
                                                @method('DELETE')
