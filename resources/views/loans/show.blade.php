        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-100 mb-6">
                <div class="px-6 py-6 border-b border-gray-200">
                    <h3 class="text-2xl font-bold text-gray-900">Loan Application #{{ $loan->id }}</h3>
                    <p class="text-gray-600 text-sm mt-1">Submitted on {{ $loan->application_date?->format('M d, Y') }}</p>
                </div>

                <div class="px-6 py-6 grid grid-cols-2 gap-6">
                    <div>
                        <p class="text-sm text-gray-600 font-semibold">Applicant Name</p>
                        <p class="text-lg text-gray-900">{{ $loan->applicant_full_name }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600 font-semibold">Borrower Account</p>
                        <p class="text-lg text-gray-900">{{ $loan->user?->name }}</p>
                    </div>

                    <div>
                        <p class="text-sm text-gray-600 font-semibold">Loan Amount</p>
                        <p class="text-lg font-bold text-green-600">UGX {{ number_format($loan->amount, 2) }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-lg text-gray-900">{{ str($loan->repayment_schedule)->replace('_', ' ')->title() }}</p>
                    </div>
                ...
                <div class="px-6 py-6">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        @foreach ($documents as $field => $label)
                            @if ($loan->$field)
                                <a href="{{ route('loans.download', [$loan->id, $field]) }}" class="flex items-center px-4 py-3 bg-blue-50 hover:bg-blue-100 rounded-lg border border-blue-200 transition">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-100">
                <div class="px-6 py-6">
                    ...
                    <div class="flex gap-3">
                        <form action="{{ route('applications.approve', $loan->id) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="px-6 py-2 bg-green-500 text-white rounded hover:bg-green-600 font-semibold">
                                ✓ Approve
                            </button>
                        </form>
                        <form action="{{ route('applications.reject', $loan->id) }}" method="POST" class="inline">
                            @csrf
            ...
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-2xl border border-gray-100">
                ...
                <div class="px-8 py-8 bg-blue-50">
                    ...
                    <div class="flex gap-4">
                        <form action="{{ route('loans.issue', $loan) }}" method="POST" class="inline">
                            @csrf
